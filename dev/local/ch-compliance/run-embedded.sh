...
# (docker-compose.embedded.yml) with internal/scarecrow enabled (oteldb.embedded.yml), instead of
# the ClickHouse/fork-engine combo run.sh exercises.
...
docker compose -f docker-compose.embedded.yml up -d --remove-orphans --build --force-recreate
