...
func run(ctx context.Context) error {
	var (
		...
		storageDir    = flag.String("storage-dir", "", "Base data directory (storage.dir) of a single-node file backend, instead of -storage-config; must not be a running node's — the open sweeps orphaned part objects (storage backend)")
