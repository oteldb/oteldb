...
func DescribeConfig[R any](s *figureout.Schema[R], cfg *Config) {
	...
	figureout.OptionalPtr(s, &cfg.MaxQueryBytes, "max_query_bytes")
	...
	// ReadOnly is a tool's argument, not a setting: odbbackup sets it on the struct it opens the
	// data directory with, and it has no serialized name to write in a file.
	figureout.Ignore(s, &cfg.ReadOnly,
		figureout.Reason("set by a tool that opens the engine, never by a config file"))
}
