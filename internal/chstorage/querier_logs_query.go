...
// SampleHopQuery defines a hop-windowed sample query used to offload
// LogQL rate/bytes_rate computation to ClickHouse.
type SampleHopQuery struct {
	Start, End     time.Time
...
type sampleHopQueryColumns struct {
	StepTS proto.ColDateTime
	Value  proto.ColFloat64
	Labels *proto.ColMap[string, string]
}

// Execute executes the hop-windowed query using the given querier.
//
// The returned steps are ordered by timestamp ascending.
// Each step contains one [logqlmetric.Sample] per label group.
func (v *SampleHopQuery) Execute(ctx context.Context, q *Querier) (_ []logqlmetric.Step, rerr error) {
	table := q.tables.Logs

	ctx, span := q.tracer.Start(ctx, "chstorage.logs.SampleHopQuery.Eval",
		trace.WithAttributes(
			xattribute.UnixNano("logql.range.start", v.Start),
			xattribute.UnixNano("logql.range.end", v.End),
			attribute.Stringer("logql.sampling", v.Sampling),
			attribute.String("logql.range_width", v.RangeWidth.String()),
			attribute.String("logql.step", v.Step.String()),
			xattribute.StringerSlice("logql.grouping_labels", v.GroupingLabels),
			xattribute.StringerSlice("logql.label_matchers", v.Sel.Labels),
			xattribute.StringerSlice("logql.line_matchers", v.Sel.Line),
			xattribute.StringerSlice("logql.label_predicates", v.Sel.PipelineLabels),
	...
	// Build label map expression for GROUP BY (same pattern as SampleQuery).
	entries := make([]chsql.Expr, 0, len(v.GroupingLabels)*2)
	for _, key := range v.GroupingLabels {
		label := string(key)
		...
		switch label {
		case logstorage.LabelSeverity, logstorage.LabelDetectedLevel:
			labelExpr = chsql.ArrayElement(severityMapExpr, chsql.Ident("severity_number"))
		default:
			var ok bool
			labelExpr, ok = q.getMaterializedLabelColumn(label)
			if !ok {
	...
	var aggExpr chsql.Expr
	switch v.Sampling {
	case CountSampling:
		aggExpr = chsql.Function("count")
	case BytesSampling:
		aggExpr = chsql.Function("sum", chsql.Length(chsql.Ident("body")))
	default:
		return nil, errors.Errorf("unexpected sampling op: %v", v.Sampling)
	}

	var (
		prec     = chsql.IntervalSecond
		stepLit  = chsql.MakeIntervalLit(int64(v.Step.Seconds()), prec)
		rangeLit = chsql.MakeIntervalLit(int64(v.RangeWidth.Seconds()), prec)

		datetime = chsql.ToDateTime(chsql.Ident("timestamp"))
		columns  = sampleHopQueryColumns{
			Labels: proto.NewMap(
				new(proto.ColStr),
				new(proto.ColStr).LowCardinality(),
			),
		...
		query = chsql.Select(table,
			chsql.ResultColumn{Name: "step_ts", Expr: chsql.Ident("hop_end"), Data: &columns.StepTS},
			chsql.ResultColumn{Name: "value", Expr: chsql.ToFloat64(aggExpr), Data: &columns.Value},
			chsql.ResultColumn{Name: "labels", Expr: chsql.Ident("labels_key"), Data: columns.Labels},
		).
			With("hop_start", chsql.HopStart(datetime, stepLit, rangeLit)).
			With("hop_end", chsql.HopEnd(datetime, stepLit, rangeLit)).
			With("labels_key", chsql.Map(entries...)).
			Where(chsql.InTimeRange("timestamp", v.Start.Add(-v.RangeWidth), v.End, proto.PrecisionNano)).
			GroupBy(chsql.Ident("labels_key"), chsql.Ident("hop_start"), chsql.Ident("hop_end")).
			Order(chsql.Ident("hop_end"), chsql.Asc)
	)
	v.Sel.addPredicates(ctx, query, mapping, q)

	rangeSecs := v.RangeWidth.Seconds()
	var (
		steps      []logqlmetric.Step
		totalSteps int
	)
	if err := q.do(ctx, selectQuery{
		Query: query,
		OnResult: func(ctx context.Context, block proto.Block) error {
			for i := 0; i < columns.StepTS.Rows(); i++ {
				stepTS := columns.StepTS.Row(i)
				value := columns.Value.Row(i) / rangeSecs
				// FIXME(tdakkota): allocates unnecessary map.
				lbls := columns.Labels.Row(i)

				ts := otelstorage.NewTimestampFromTime(stepTS)
				sample := logqlmetric.Sample{
					Data: value,
					Set:  logqlabels.AggregatedLabelsFromMap(lbls),
				}

				if len(steps) > 0 && steps[len(steps)-1].Timestamp == ts {
					steps[len(steps)-1].Samples = append(steps[len(steps)-1].Samples, sample)
				} else {
					steps = append(steps, logqlmetric.Step{
						Timestamp: ts,
						Samples:   []logqlmetric.Sample{sample},
					})
					totalSteps++
				}
			}
	...
	span.AddEvent("steps_fetched", trace.WithAttributes(
		attribute.Int("chstorage.total_steps", totalSteps),
	))
