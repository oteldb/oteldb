	...
	cfg := restoreTable{
		...
		NewColumns: func() ([]proto.Input, Columns, func(), func() int) {
			var (
				...
				rows = func() int {
					return sc.start.Rows()
				}
			)
			return []proto.Input{sc.Input(), tags.Input()}, columns, add, rows
		},
		Logger: r.logger,
	}
	return cfg.Do(ctx, dir, []string{r.tables.Spans, r.tables.Tags}, r.client)
}

func convertSpanEvents(
